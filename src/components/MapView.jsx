...
const getDisasterCenter = (scenarioId) => {
  if (scenarioId === 'WZ-Kerala') return [9.9816, 76.2999]; // Ernakulam
  if (scenarioId === 'WZ-Konkan') return [11.9, 75.3]; // Kannur/North Kerala
  if (scenarioId === 'WZ-Northeast') return [11.5, 76.0]; // Wayanad
  ...
  return (
    <div className="flex-1 h-full w-full relative z-0">
      ...
      >
        ...
        {/* Draw Glowing Rail Geometry from OSM */}
        {corridor.length > 0 ? (
          corridor.map((segment, i) => (
            <React.Fragment key={`corridor-group-${i}`}>
              {/* Thick soft background neon glow */}
              <Polyline 
                positions={segment}
                color="#3B82F6"
                weight={4}
              ...
              <Polyline 
                ...
                weight={1.5}
                opacity={0.8}
                pathOptions={{ className: 'railway-glow' }}
              />
            </React.Fragment>
          ))
        ) : (
          /* Hardcoded fallback track segment if OSM loading fails/rate limits */
          stations.length > 0 && (
            <Polyline
              positions={stations.sort((a,b)=>b.lat-a.lat).map(s=>[s.lat, s.lng])}
              color="#3B82F6"
              weight={2.5}
              opacity={0.7}
              pathOptions={{ className: 'railway-glow' }}
