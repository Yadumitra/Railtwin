...
async function getKeralaCorridor() {
  ...
  try {
    ...
    const segments = res.data.elements.map(way => way.geometry.map(pt => [pt.lat, pt.lon]));
    if (segments.length > 0) return segments;
  } catch(err) {
    console.error('OSM Corridor Error. Using local cached fallback rails:', err.message);
  }
  return OFFLINE_CORRIDOR;
}

module.exports = { getKeralaStations, getKeralaCorridor, OFFLINE_STATIONS, OFFLINE_CORRIDOR };
